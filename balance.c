...
uint32_t get_forward_worker_id(struct rte_mbuf *pkt, uint32_t n_workers)
{
    ...
    if (ret >= 0)
    {
        ...
        {
            printf("\nflow hash lookup failed in worker %d \n", worker_id);
...
//返回所有节点的负载值之和
static uint32_t decrease_sort_worker(struct app_worker_load_info *workers_sorted[])
{
    uint32_t worker_num = app.n_workers;
    uint32_t worker_id, load_sum;
    //初始化workers_sorted[]
...
*/
static int classify_worker(uint32_t *load_heavy_index, double load_heavy_incr, uint32_t *load_av,
                           struct app_worker_load_info *workers_sorted[], uint32_t worker_num)
{
    ...
    //降序排序worker
    *load_av = decrease_sort_worker(workers_sorted);
                          ...
                          struct app_worker_load_info *workers_sorted[], uint32_t worker_num)
{
    ...
    for (;;)
    {
        ...
        //选取待迁移的流
        for (hvy_flow_id = heavy_worker->n_flows - 1; hvy_flow_id >= 0; hvy_flow_id--)
        {
            heavy_flow = &heavy_worker->worker_flow_ld_info[hvy_flow_id];

            //如果是已经迁移走的流，则继续寻找
            if (heavy_flow->rx_pkts_num == 0)
            {
                continue;
            }
            ...
            for (lit_worker_id = worker_num - 1; lit_worker_id > load_heavy_index; lit_worker_id--)
            {
                ...
                if (light_worker->worker_load_value + heavy_flow->flow_load_value < load_heavy_incr)
                {
                    //找到第一个可以接收这条流的轻负载节点
                    break;
                }
            ...
            if (lit_worker_id == load_heavy_index)
            {
                ...
                // dest_ip.s_addr = heavy_flow->tpl.dst_ip;
                // printf("\nheavy_worker_id = %d, heavy_worker_ld_value = %d\n", heavy_worker->worker_id, heavy_worker->worker_load_value);
                // printf("\nstop at flow.dstIp = %s , flow.value = %d \n", inet_ntoa(dest_ip), heavy_flow->flow_load_value);

                //循环查找重负载节点上的流，知道找到一个合适的
                continue;
            }
            else
            {
        ...
        //重负载节点上的所有流都没有轻负载节点可以接收，只能放弃这个重负载节点，继续操作下一个重负载节点
        if (hvy_flow_id == -1)
        {
            //debug
            printf("\nheavy worker id = %d is still heavy\n", heavy_worker->worker_id);
            load_balance_res = -1;
            hvy_worker_id++;
            continue;
        }
        ...
        if (ret >= 0)
        {
            ...
            // struct in_addr dest_ip;
            // dest_ip.s_addr = heavy_flow->tpl.dst_ip;
            // printf("\nbefore transfer flow(dstIP=%s), it is on worker %d\n", inet_ntoa(dest_ip), p_h_val->worker_id);
        ...
        // dest_ip.s_addr = heavy_flow->tpl.dst_ip;
        // ret = rte_hash_lookup_data((const struct rte_hash *)app.flow_redirect_info_table, (const void *)trans_flow_touple, (void **)&p_h_val);
        // printf("\n after transfer flow(dstIP=%s), it is on worker %d\n", inet_ntoa(dest_ip), p_h_val->worker_id);
        ...
        //清空重负载节点上待迁移流的存储空间
        memset(heavy_flow, 0, sizeof(sizeof(struct app_flow_load_info)));

        //更新两个节点的n_flows
        struct app_flow_load_info *valid_flow;
        for (hvy_worker_flow_edge = heavy_worker->n_flows - 1; hvy_worker_flow_edge >= 0; hvy_worker_flow_edge--)
        {
            valid_flow = &heavy_worker->worker_flow_ld_info[hvy_worker_flow_edge];

            if (valid_flow->rx_pkts_num != 0)
            {
                hvy_worker_flow_edge++;
                break;
            }
        }

        if (hvy_worker_flow_edge == -1)
        {
            hvy_worker_flow_edge = 0;
        }
        heavy_worker->n_flows = hvy_worker_flow_edge;
...
{
    //计算所有节点的节点负载值
    uint32_t i, load_heavy_index, load_av;
    uint64_t load_heavy_incr = app.heavy_load_threshold;
    ...
    //分类节点
    ret = classify_worker(&load_heavy_index, load_heavy_incr, &load_av,
                          workers_sorted, app.n_workers);

    if (load_heavy_incr < load_av)
        load_heavy_incr = load_av;
