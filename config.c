...
{
    struct app_lcore_params *lp;
    uint32_t lcore, lcore_start, lcore_end;
    const char *p = arg;
    uint32_t n_tuples;
    ...
    if (*p == '[')
    {
        ...
        errno = 0;
        lcore_end = strtoul(p, NULL, 0);
        if ((errno != 0))
        {
            return -2;
        }

        for (lcore = lcore_start; lcore <= lcore_end; lcore++)
        {
...
static int
parse_heavy_td(const char *arg)
{
    int ret = 0;
    uint64_t heavy_ld_td;
    heavy_ld_td = strtoul(arg, NULL, 0);

    if (heavy_ld_td > 1000)
    {
        ret = -1;
        printf("\nheavy load  threshold too big\n");
    }
    app.heavy_load_threshold = heavy_ld_td;
    return ret;
}

static uint32_t
parse_dynamic(const char *arg)
{
    int ret = 0;
    uint64_t heavy_ld_td;
    heavy_ld_td = strtoul(arg, NULL, 0);
...
int app_parse_args(int argc, char **argv)
{
    ...
    uint32_t arg_rsz = 0;
    uint32_t arg_bsz = 0;
    uint32_t f_present = 0;

    argvopt = argv;

    while ((opt = getopt_long(argc, argvopt, "p:u:h:d:f:",
                              lgopts, &option_index)) != EOF)
    {
        ...
        case 'u':
            ...
            break;
        case 'h':
            if (parse_heavy_td(optarg))
            {
                printf("parsing heavy load  threshold \"%s\" failed\n",
                       optarg);
                return -1;
            }
    ...
    if ((arg_rx == 0) || (arg_tx == 0) || (arg_w == 0) || (f_present == 0))
    {
