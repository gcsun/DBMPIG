...
void parse_sp6_tokens(char **tokens, uint32_t n_tokens,
                      struct parse_status *status)
{
    ...
    for (ti = 2; ti < n_tokens; ti++)
    {
        ...
        if (strcmp(tokens[ti], "proto") == 0)
        {
            ...
            APP_CHECK(parse_range(tokens[ti], &low, &high) == 0, status, "unrecognized input \"%s\""
                                                                         ", expect \"from:to\"",
                      tokens[ti]);
            if (status->status < 0)
                return;
...
print_one_ip6_rule(const struct acl6_rules *rule, int32_t extra)
{
    ...
    printf(":%.2x%.2x:%.2x%.2x/%u ", a, b, c, d,
           rule->field[IP6_SRC0].mask_range.u32 + rule->field[IP6_SRC1].mask_range.u32 + rule->field[IP6_SRC2].mask_range.u32 + rule->field[IP6_SRC3].mask_range.u32);
    ...
    uint32_t_to_char(rule->field[IP6_DST3].value.u32,
                     &a, &b, &c, &d);
    printf(":%.2x%.2x:%.2x%.2x/%u ", a, b, c, d,
           rule->field[IP6_DST0].mask_range.u32 + rule->field[IP6_DST1].mask_range.u32 + rule->field[IP6_DST2].mask_range.u32 + rule->field[IP6_DST3].mask_range.u32);
...
void sp6_init(struct socket_ctx *ctx, int32_t socket_id)
{
    const char *name;
